...
lasso_reg = MultiTaskLassoCV(alphas=alphas, cv=5, max_iter=10000)
...
for fold, (train_index, test_index) in enumerate(k_fold.split(X)):
    ...
    # Train lassoCV to find the best alpha
    lasso_reg.fit(X_train, Y_train)
    best_alpha = lasso_reg.alpha_
    
    # Train lasso with the best alpha
    lasso_final = MultiTaskLassoCV(alphas=[best_alpha])
    lasso_final.fit(X_train, Y_train)
    
    # Make predictions
    train_pred = lasso_final.predict(X_train)
    ...
    np.save('results/raw-results/lasso/lasso-train_pearson_fold_{}.npy'.format(fold + 1), train_pearson)
    np.save('results/raw-results/lasso/lasso-test_pearson_fold_{}.npy'.format(fold + 1), test_pearson)
    ...
    train_predictions_df.to_csv(f'results/raw-results/lasso/fold_{fold + 1}_train_predictions.csv')
    test_predictions_df.to_csv(f'results/raw-results/lasso/fold_{fold + 1}_test_predictions.csv')
...
fold_info_df.to_csv('results/lasso-kfold_cv_results.csv')