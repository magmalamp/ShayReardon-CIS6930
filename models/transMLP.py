...
from tensorflow.keras.layers import BatchNormalization, MultiHeadAttention
...
def build_mlp(input_shape, output_shape):
    ...
    # **Project input into an embedding space**
    x = layers.Dense(4096, activation="relu")(inputs)  
    x = layers.LayerNormalization()(x)
    x = layers.Dropout(0.2)(x)
    
    # **Transformer Encoder**
    x = layers.Reshape((1, 4096))(x)  # Reshape for Transformer
    x = layers.MultiHeadAttention(num_heads=4, key_dim=64)(x, x, x)
    x = layers.Flatten()(x)  
    ...
    outputs = layers.Dense(output_shape, activation='linear')(x)  # Linear activation for regression
...
for fold, (train_index, test_index) in enumerate(k_fold.split(X_np)):
    ...
    transmlp = build_mlp(input_shape=X_train.shape[1], output_shape=Y_train.shape[1])
    
    # Train the MLP model
    transmlp.fit(X_train, Y_train, validation_split=0.2, epochs=200, batch_size=32, verbose=1)
    
    # Make predictions on train and test sets
    train_pred = transmlp.predict(X_train)
    test_pred = transmlp.predict(X_test)

    print("y_true variance: ", np.var(Y_test, axis=0))
    print("y_pred variance: ", np.var(test_pred, axis=0))
    print("y_pred unique values: ", np.unique(test_pred))
    ...
    np.save('results/raw-results/transmlp/transmlp-train_pearson_fold_{}.npy'.format(fold + 1), train_pearson)
    np.save('results/raw-results/transmlp/transmlp-test_pearson_fold_{}.npy'.format(fold + 1), test_pearson)
    ...
    train_predictions_df.to_csv(f'results/raw-results/transmlp/fold_{fold + 1}_train_predictions.csv')
    test_predictions_df.to_csv(f'results/raw-results/transmlp/fold_{fold + 1}_test_predictions.csv')
...
fold_info_df.to_csv('results/transmlp-kfold_cv_results.csv')