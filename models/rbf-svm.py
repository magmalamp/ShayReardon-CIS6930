...
param_grid = {
    ...
    'estimator__epsilon': [0.01, 0.1],  # Epsilon-tube width
    'estimator__gamma': ['auto']  # Kernel coefficient
}
...
for fold, (train_index, test_index) in enumerate(k_fold.split(X)):
    ...
    # Train svmCV to find the best alpha
    svr = MultiOutputRegressor(SVR(kernel='rbf'))
    grid_search = GridSearchCV(svr, param_grid, scoring='r2', cv=3, n_jobs=2, verbose=1)
    ...
    np.save('results/raw-results/svm/svm-train_pearson_fold_{}.npy'.format(fold + 1), train_pearson)
    np.save('results/raw-results/svm/svm-test_pearson_fold_{}.npy'.format(fold + 1), test_pearson)
    ...
    train_predictions_df.to_csv(f'results/raw-results/svm/fold_{fold + 1}_train_predictions.csv')
    test_predictions_df.to_csv(f'results/raw-results/svm/fold_{fold + 1}_test_predictions.csv')
...
fold_info_df.to_csv('results/svm-kfold_cv_results.csv')